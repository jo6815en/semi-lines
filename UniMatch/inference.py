...
def inference(model, loader, cfg, is_wireframe=False, save_img=None):
    ...
    with torch.no_grad():
        for batch_data in data_iter:
            ...
            if count % 300 == 0:
                image_np = imgs[0, :, :, :].cpu().squeeze(0).permute(1, 2, 0).numpy()
                mask_np = batch_outputs[0, :, :, :].cpu().squeeze(0).numpy()

                outputs = batch_outputs[0:1, :, :, :]

                image = cv2.cvtColor(image_np, cv2.COLOR_BGR2RGB)

                lines = get_pred_lines(image, outputs, model, [512, 512])
                # print("Lines: ", lines.shape)
                img_temp = img_org
                for l in lines:
                    cv2.line(img_temp[0], (int(l[0]), int(l[1])), (int(l[2]), int(l[3])), (0, 200, 200), 1, 16)

                #plt.imshow(img_temp[0])
                #plt.show()

                # if save_img:
                #    temp_id = id[0].split()[0].split('/')[2].split('.')[0]
                #    overlay_np = overlay.astype(np.uint8)
                #    cv2.imwrite(os.path.join(save_img, f'{temp_id}_result.jpg'), overlay)
