...
def evaluate(model, loader, cfg, epoch, logger, is_wireframe, save_img=None):
    model.eval()
    ...
    with torch.no_grad():
        ...
        sAP = AP(tp, fp) * 100
        logger.info("==>step: {}, f_score: {}, recall: {}, precision: {}, sAP10: {}\n ".
...
def main():
    ...
    if os.path.exists(os.path.join(args.save_path, 'latest.pth')):
        ...
        optimizer.load_state_dict(checkpoint['optimizer'])
        if rank == 0:
            logger.info('****** Load checkpoint from earlier ******')
    ...
    for epoch in range(epoch + 1, cfg.train.num_train_epochs):
        if rank == 0:
            logger.info('===========> Epoch: {:}, LR: {:.5f}, Previous best: {:.2f}, Previous sAP10: {:.2f}, epoch: {:.2f}'.format(
        ...
        if epoch % 10 == 0:
            m = evaluate(model, valloader, cfg, epoch, logger, is_wireframe=args.is_wireframe, save_img='out')
