    ...
    def __init__(self, cfg, mode, id_path=None, nsample=None, is_train=True):
        ...
        if mode == 'train_l':
            ...
            with open('splits/%s/%s.txt' % (self.name, self.mode), 'r') as f:
                ...
                self.anns = self._load_anns(self.img_dir, self.label_fn, self.ids)
                print("When loading the validation: ", len(self.ids))
        elif mode == 'val_sam_segs':
            with open('splits/%s/val_sam_segs.txt' % self.name, 'r') as f:
                self.ids = f.read().splitlines()
                self.img_dir = cfg.samsegs.img_dir
                self.label_fn = cfg.samsegs.label_fn
                self.anns = self._load_anns(self.img_dir, self.label_fn, self.ids)
                print("When loading the validation: ", len(self.ids))
        elif mode == 'val_skrylle':
            with open('splits/%s/val_skrylle.txt' % self.name, 'r') as f:
                self.ids = f.read().splitlines()
                self.img_dir = cfg.skrylle.img_dir
                self.label_fn = cfg.skrylle.label_fn
                self.anns = self._load_anns(self.img_dir, self.label_fn, self.ids)
                print("When loading the validation: ", len(self.ids))
        elif mode == 'test':
            with open('splits/%s/val_katam.txt' % self.name, 'r') as f:
                self.ids = f.read().splitlines()
